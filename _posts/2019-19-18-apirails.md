...
Las URL son localizadores (de ahí el nombre). Una URL es la dirección de su casa: Calle Whatever , 105, Medellin.
Las URN son nombres. Para ud, una URN es tu nombre: Andres Bernal.
Los URI pueden ser un localizador (URL), un nombre (URN) o ambos. Básicamente, todas las URL son URI, pero no todas las URI son URL.

Lo único que puede calificar un URI como URL es si este último incluye cómo encontrar el primero. www.google.com/account no es una URL, solo un URI. Sin embargo, http://www.google.com/account es una URL (y sigue siendo un URI) porque incluye el protocolo (http: //) que se puede usar para acceder al recurso (www.google.com/account) .
...
> https://www.ruby-lang.org/usuarios
...
Pero también debemos considerar que estamos construyendo para los seres humanos y todavía estamos lejos de eliminar a los desarrolladores de la ecuación. Dado que estamos creando API web para humanos, que los usarán para implementar su código de cliente, debemos usar conceptos y semánticas que puedan entender. Esto sigue el mismo principio que se usa para nombrar recursos para sitios web.

Lo siguiente se basa en las mejores prácticas para construir una API fácil de usar, y no realmente en los estándares web.

Si quiero representar el concepto de una lista de usuarios, simplemente usaría la palabra pluralizada usuarios, que es solo un sustantivo. Hacer una solicitud a esta URL devolvería una representación que enumera un grupo de usuarios. Cualquiera podría entender esto, y ver la representación solo confirmará su idea sobre este recurso.

Con la URL a continuación, obtenemos una lista de usuarios.

> https://www.ruby-lang.org/usuarios

Pero, ¿no deberíamos llamarlo https://www.ruby-lang.org/obtener_usuarios, para que la gente lo entienda más fácilmente?
...
Algunas personas prefieren usar nombres singulares para sus recursos, como usuario. Esta elección depende de usted, pero recomiendo simplemente usar la versión plural. Decida lo que decida, mantengase firme. No use / usuarios y / usuario /: id en la misma API, es confuso para los desarrolladores. No olvide su objetivo es construir algo fácil de usar. no es para ud el codigo. usaré el plural para los nombres de los recursos, ya que es el más utilizado en la comunidad Ruby.

Ahora que tenemos una API funcional minimalista, es hora de comenzar a usarla. Para hacerlo, utilizaremos curl con el que los datos se pueden transferir a través de varios protocolos que incluyen, entre otros, HTTP.

Es genial tener esta herramienta en su caja de herramientas como Sofware Developer, especialmente para las API web de un edificio. Se puede usar un navegador web para probar algunas partes de una API web, pero es demasiado limitado para ser realmente útil. De hecho, solo podemos ejecutar solicitudes GET usando un navegador, y solo si no hay un Encabezado HTTP para cambiar.

curl nos permite definir las solicitudes de la forma en que las queremos y, por lo tanto, es una gran herramienta para incluir en la documentación de API web. Dado que las solicitudes curl se pueden generar y copiar fácilmente, son una excelente manera de permitir a los desarrolladores probar su API y ver si todo funciona antes de comenzar a hacer algo.
...
* No intente consultar a usuarios que aún no existen; todavía no estamos manejando errores! Habrá una sección completa sobre eso en el próximo capítulo.
